...
async function downloadElementAsPDF({
    ...
    title = "",                 // optional text header on first page
    subtitle = "",
    ...
    subtitle?: string,
    pageFormat?: string,
    orientation?: NonNullable<ConstructorParameters<typeof jsPDF>[0]>,
}) {
    ...
    let bodyTopMM = title ? (marginMM + titleSpacingMM) : marginMM;
    if (subtitle) {
        bodyTopMM += 10;
    }
