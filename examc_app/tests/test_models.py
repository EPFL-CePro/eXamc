...
from examc_app.models import Exam, PagesGroup, Reviewer, PagesGroupComment, PageMarkers, DrawnImage
...
class ExamModelTestCase(TestCase):
    ...
    #     group1 = PagesGroup.objects.create(exam=self.exam, group_name="Group 1", page_from=1, page_to=5)
    #     group2 = PagesGroup.objects.create(exam=self.exam, group_name="Group 2", page_from=6, page_to=10)
    #
    #     self.assertEqual(self.exam.get_max_points(), 0)
    #
    #     PageMarkers.objects.create(copie_no="1", page_no="1", exam=self.exam, pages_group=group1, filename="file1.pdf")
    #     PageMarkers.objects.create(copie_no="2", page_no="6", exam=self.exam, pages_group=group2, filename="file2.pdf")
    #
    #     self.assertEqual(self.exam.get_max_points(), 0)
...
class PagesGroupCommentTestCase(TestCase):
    ...
    def test_exam_pages_group_comment(self):
        comment = PagesGroupComment.objects.create(user=self.user, pages_group=self.group, content="Test Comment", is_new=True)
...
class PageMarkersTestCase(TestCase):
    ...
    def test_scan_markers_str_method(self):
        marker = PageMarkers.objects.create(copie_no="1", page_no="1", exam=self.exam, pages_group=self.group, filename="file1.pdf")
