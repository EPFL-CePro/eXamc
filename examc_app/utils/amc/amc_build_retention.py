"""
AMC build retention policy.

This module decides which ``ExamBuild`` rows should be kept and which can be
removed. The policy is intentionally conservative:

- always protect builds that are locked or already used downstream
- always keep the latest ready build
- keep a rolling window of recent ready and non-ready builds
- optionally delete associated non-shared files when a build is removed

The goal is to limit storage growth without breaking scan, scoring, or audit
...
class BuildRetentionDecision:
    """
    Result of the retention policy for one build.

    Attributes:
        build_id: Database id of the build.
        exam_id: Owning exam id.
        version: Build version number.
        status: Build status at decision time.
        keep: Whether the build must be kept.
        reasons: Human-readable reasons explaining the decision.
    """
    build_id: int
    exam_id: int
    version: int
...
def _build_keep_reasons(build: ExamBuild) -> list[str]:
    """
    Return hard-protection reasons for a build.

    A build is considered non-deletable when it is explicitly locked or when it
    already has downstream data attached, such as copies, scan pages, or score
    results.
    """
    reasons = []
...
def get_exam_build_retention_decisions(exam, *, exclude_build_id: int | None = None):
    """
    Compute keep/delete decisions for all builds of an exam.

    Policy summary:
    - keep builds with hard-protection reasons
    - keep the latest ready build
    - keep the last ``KEEP_SUCCESSFUL`` ready builds
    - keep the last ``KEEP_NON_READY`` non-ready builds
    - optionally keep one explicitly excluded build id

    Args:
        exam: Exam instance whose builds should be evaluated.
        exclude_build_id: Optional build id to force-keep for safety.

    Returns:
        list[BuildRetentionDecision]: One decision object per build, ordered by
        newest first.
    """
...
def _delete_build_files(build: ExamBuild) -> dict:
    """
    Delete non-shared files referenced by a build.

    Files inside the shared persistent project folder are intentionally skipped,
    because that folder represents the current canonical AMC project for the
    exam and may still be referenced by newer builds.

    Returns:
        dict: Deleted paths and deletion failures.
    """
    deleted = []
    failed = []
...
def delete_exam_build(build: ExamBuild, *, delete_files: bool = True) -> dict:
    """
    Delete one build if it is not protected by downstream usage.

    Args:
        build: Build to delete.
        delete_files: Whether referenced non-shared files should be removed.

    Returns:
        dict: Deletion result, including reasons when deletion is refused.
    """
...
def cleanup_exam_builds(
    ...
    *,
    ...
    """
    Apply the retention policy to all builds of an exam.

    When ``dry_run`` is enabled, this function returns the planned keep/delete
    decisions without deleting anything. This is useful for admin previews and
    testing.

    Args:
        exam: Exam whose builds should be cleaned.
        dry_run: If ``True``, do not perform deletions.
        delete_files: Whether to delete referenced non-shared files too.
        exclude_build_id: Optional build id to protect during cleanup.

    Returns:
        dict: Summary containing kept builds, deletable builds, and actual
        deletion results when not running in dry-run mode.
    """
